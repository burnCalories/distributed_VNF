...
class SPRPolicy(TQCPolicy):
    ...
    def __init__(self, observation_space, action_space, lr_schedule, **_kwargs):
        ...
        super(SPRPolicy, self).__init__(observation_space, action_space, lr_schedule=lr_schedule, optimizer_class=th.optim.Adam,
...
class TQC_Agent:
    ...
    def create_model(self, n_envs=1):
        ...
        self.model = TQC(
            ...
            target_entropy=self.params.agent_config['target_entropy'],
            top_quantiles_to_drop_per_net=self.params.agent_config['top_quantiles_to_drop_per_net'],
            sde_sample_freq=self.params.agent_config['sde_sample_freq'],
    ...
    def load_model(self, path=None):
        ...
        if path is not None:
            self.model = TQC.load(path)
        else:
            self.model = TQC.load(self.params.model_path)
